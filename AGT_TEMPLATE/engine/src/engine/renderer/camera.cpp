...
void engine::perspective_camera::set_view_matrix_custom(glm::vec3 position, glm::vec3 look_at,
                                                        glm::vec3 objectPosition,const timestep& timestep) {
    // m_position = position;
    // m_front_vector = glm::normalize(look_at - position);
    // m_view_mat = glm::lookAt(m_position, m_position + m_front_vector, m_up_vector);
    // update_camera_vectors();
    ...
    // glm::mat4 view;
    float yaw = x * s_mouse_sensitivity * timestep;
    float pitch = y * s_mouse_sensitivity * timestep;

    
    m_view_mat = glm::lookAt(glm::vec3(camX, 0.0, camZ), glm::vec3(yaw, 0.0, pitch), glm::vec3(0.0, 1.0, 0.0));

   
    // m_view_mat = glm::translate(m_view_mat, glm::vec3(0, 2.f, radius));
    // m_view_mat = glm::rotate(m_view_mat, 1.f,glm::vec3(pitch, yaw, 0));
    // m_view_mat = glm::translate(m_view_mat, objectPosition);
