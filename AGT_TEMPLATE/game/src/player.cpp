...
void Player::menuUpdateCamera(engine::perspective_camera& camera, const engine::timestep& timestep, bool hasStarted) {
    this->hasStarted = hasStarted;
    auto menuPosition = glm::vec3(0, 15.5f, 0);
    glm::vec3 menuForward;
    menuForward = mObject->forward();
    ...
    if(hasStarted) {
        ...
        glm::vec3 cam_pos = menuPosition - menuForward * B;
        cam_pos.y += A;

        glm::vec3 cam_look_at = menuPosition + menuForward * C;
        cam_look_at.y = 0.f;
        // camera.set_view_matrix_custom(cam_pos, cam_look_at, mObject->position(), timeStep);
        camera.set_view_matrix(cam_pos, cam_look_at);
...
void Player::updateCamera(engine::perspective_camera& camera, const engine::timestep& timestep,bool hasStarted) {
    ...
    if (hasStarted) {
        ...
        menuUpdateCamera(camera, timestep, hasStarted);
    }
}
